# Phase 1 Pair Reviewed Subset

이 파일은 `why-we-no-longer-evaluate-swe-bench-verified` 쌍에서 유지보수자 검토가 완료된 paragraph-level reviewed subset입니다.

- 목적: Phase 1 공식 EN-KO 문서 쌍에서 maintainer-reviewed example을 보존하고, `docs/golden/`으로 승격할 source-of-truth로 사용합니다.
- 선택 배경: original improved eval 기준 낮은 overall score, 남아 있는 issue, 낮은 backtranslation score를 함께 고려해 검토 대상을 추렸습니다.
- 상태: 아래 항목은 모두 `decision: manual_rewrite_approved`로 확정된 현재 approved reviewed subset입니다.
- `reviewed_golden_ko (approved)`: 유지보수자가 승인한 한국어 target이며, 현재 paragraph-level `reviewed_golden` 승격 원문으로 사용합니다.

## why-we-no-longer-evaluate-swe-bench-verified.block-013

- decision: `manual_rewrite_approved`
- overall_score: `90.6`
...
### reviewed_golden_ko (approved)

모델에는 테스트 코드가 제공되지 않으며, 원본 이슈 내용과 수정 전 저장소 상태만을 바탕으로 코드 변경을 생성해야 합니다. 변경 사항을 적용한 후 모든 테스트를 통과해야만 해당 문제가 해결된 것으로 간주됩니다.

### notes

- 의미는 정확하지만 `"테스트 코드"`가 단순히 `"테스트"`로 완화되어 약간 덜 구체적입니다.
- 유지보수자 승인 완료. 현재 paragraph-level `reviewed_golden` subset으로 승격된 문안입니다.
...
새로운 분석 결과, Verified 세트에서 두 가지 주요 문제가 확인되었습니다. 이는 이 벤치마크가 현재 성능 수준에서 프런티어 모델 출시에 필요한 자율 소프트웨어 엔지니어링 역량 향상을 측정하는 데 더 이상 적합하지 않음을 시사합니다.
...
환경 설정(예: Linux와 Windows의 차이 또는 Python 버전)에 따라 일부 테스트는 실제 오류가 없는데도 실패할 수 있습니다.
...
나머지 5.1%의 작업에는 이 분류 기준에 명확히 속하지 않는 문제가 있었습니다.
...
수정과 무관한 기존 기능이 유지되는지를 확인하기 위해, 수정 전후 모두 통과해야 하는 회귀 테스트입니다.
...
Gemini 3 Flash는 작업 ID 외에 추가 정보가 주어지지 않았음에도, 작업 설명과 골드 패치의 세부 내용을 그대로 출력할 수 있습니다. 여기에는 사용자 이름 유효성 검사를 위한 새로운 정규식과 변경 사항의 정확한 줄 번호가 포함됩니다.
